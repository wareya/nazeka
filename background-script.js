...
let epwing = [
// Fill this out with some testing data from a json file generated by zero-epwing so that it looks something like this screenshot: https://i.imgur.com/waTYH1i.png
];
...
// FIXME: this is almost certainly only the format of the kenkyuusha epwing dictionary. others might be completely different.
            ...
            if(entry.k_ele)
            {
                for(let spelling of entry.k_ele)
                {
                    ...
                    if (lookup_epwing_kan.has(text))
                    {
                        ...
                        {
                            if(epwing[wing] && epwing[wing]["text"])
                            {
                                if(epwing_data == "")
                                {
                                    epwing_data = epwing[wing]["text"];
                                    // FIXME: using .includes() is too loose. just simpler than adding real arrays of spellings/readings to the epwing data on load.
                                    exact = (epwing[wing]["heading"].includes(foundtext) || epwing[wing]["text"].includes(foundtext));
                                }
                                else if(!exact && (epwing[wing]["heading"].includes(foundtext) || epwing[wing]["text"].includes(foundtext)))
                                {
                                    epwing_data = epwing[wing]["text"];
                                    exact = true;
                                }
